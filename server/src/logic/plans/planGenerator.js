...
// Generators
// --------------------------------
function assignRunWorkouts(days, profile) {
    ...
    let ratio = GOAL_RATIOS[profile.goal];
    if (!ratio) {
        throw new Error(`Invalid goal type: ${profile.goal} for run workout generation.`);
    }
    ...
    // Adjust for level
    let adjustment = Math.round(workoutMiles * 0.1);
...
function assignLiftWorkouts(days, profile) {
    let totalLiftDays = profile.liftDaysPerWeek;
    // Only subtract if a long run exists AND overlaps a lift day
    const liftDaysIncludesLongRun = profile.liftDays.includes(profile.longRunDay);
    if (liftDaysIncludesLongRun) {
        totalLiftDays -= 1;
    }
    // Only allow 1 leg day per week
    let maxLowerDays = 1;
    let liftsAssigned = 0;
    let lowerAssigned = 0;
    let lastLiftTypeByDay = {};
    // Find all eligible lift days
    const eligibleLiftDays = profile.liftDays.sort((a, b) => {
    ...
    for (const { day } of legCandidates) {
        if (lowerAssigned >= maxLowerDays)
            break;
        if (liftsAssigned >= totalLiftDays)
            break;
    ...
    const pushCount = days.reduce((sum, d) => sum + d.workouts.filter(w => w.type === "lift" && w.liftType === "push").length, 0);
    const pullCount = days.reduce((sum, d) => sum + d.workouts.filter(w => w.type === "lift" && w.liftType === "pull").length, 0);
    // Convert to upper if one side is missing entirely
    if (pushCount === 0 && pullCount > 0) {
        days.forEach(d => {
            d.workouts = d.workouts.map(w => w.type === "lift" && w.liftType === "pull"
        ...
        days.forEach(d => {
            d.workouts = d.workouts.map(w => w.type === "lift" && w.liftType === "push"
                ? { type: "lift", liftType: "upper" }
                : w);
        });
    }
    ...
    for (const day of days) {
        let liftWorkout = day.workouts.find(w => w.type === "lift");
        if (liftWorkout) {
...
function validateProfile(profile) {
    ...
    if (profile.runDaysPerWeek > 0) {
        ...
        if (!profile.runDays.includes(profile.longRunDay)) {
        ...
        if (profile.liftDaysPerWeek > 0 && profile.liftDays.includes(profile.longRunDay)) {
    ...
    if (profile.liftDaysPerWeek > 0) {
        ...
        if (profile.runDaysPerWeek > 0 && profile.liftDaysPerWeek > 3) {
            throw new FitnessProfileError("For running-focused goals, no more than 3 lift days per week are allowed.");
        }
        if (profile.currentWeeklyMileage >= 65 && profile.liftDaysPerWeek > 2) {
            throw new FitnessProfileError("For weekly mileage of 65 or more, no more than 2 lift days per week are allowed.");
        }
...
function validateDayArray(days, label) {
...
function scoreLegDay(day, days) {
    ...
    if (hasWorkoutRunToday)
        score += 3;
    else if (hasAnyRunToday)
        score += 1;
    if (prevDayHadLong)
        score -= 3;
    if (prevDayHadWorkout)
        score -= 1;
    if (nextDayHasLong)
        score -= 3;
