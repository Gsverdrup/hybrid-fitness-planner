...
// ------------------------------------------------------------
export function generateMarathonPlan(profile) {
    // Validation
    ...
    const targetPeakMileage = calculateTargetPeakMileageMarathon(profile.startingWeeklyMileage, profile.trainingLengthWeeks, profile.runningLevel);
    ...
    for (let week = profile.trainingLengthWeeks - 1; week >= 0; week--) {
        const weekType = weekPattern.charAt(week);
    ...
    for (let week = 0; week < profile.trainingLengthWeeks; week++) {
        ...
        let curWeek = generateWeeklyPlan(profile);
...
function calculateMarathonLongRun(weeklyMileage, weeksUntilRace) {
...
function calculateTargetPeakMileageMarathon(startingMileage, trainingWeeks, level) {
    let basePeak;
...
// ------------------------------------------------------------
export function generateHalfMarathonPlan(profile) {
    // Validation
    ...
    const targetPeakMileage = calculateTargetPeakHalfMarathon(profile.startingWeeklyMileage, profile.trainingLengthWeeks, profile.runningLevel);
    const deloadFactor = profile.runningLevel === "beginner" ? 0.75 :
        profile.runningLevel === "intermediate" ? 0.80 : 0.85;
    const weeklyMileages = [];
    const longRunLengths = [];
...
function calculateHalfMarathonLongRun(weeklyMileage, weeksUntilRace) {
...
function calculateTargetPeakHalfMarathon(startingMileage, trainingWeeks, level) {
    let basePeak;
...
// 10K PLAN GENERATOR
// ------------------------------------------------------------
export function generate10kPlan(profile) {
    // Validation
    ...
    const targetPeakMileage = calculateTargetPeak10k(profile.startingWeeklyMileage, profile.trainingLengthWeeks, profile.runningLevel);
...
function calculate10kLongRun(weeklyMileage, weeksUntilRace) {
...
function calculateTargetPeak10k(startingMileage, trainingWeeks, level) {
    let basePeak;
    if (startingMileage < 15) {
...
// 5K PLAN GENERATOR
// ------------------------------------------------------------
export function generate5kPlan(profile) {
    // Validation
    if (profile.startingWeeklyMileage > 50) {
    ...
    const targetPeakMileage = calculateTargetPeak5k(profile.startingWeeklyMileage, profile.trainingLengthWeeks, profile.runningLevel);
    // Set deload factor
    const deloadFactor = profile.runningLevel === "beginner" ? 0.70 :
        profile.runningLevel === "intermediate" ? 0.75 : 0.80;
    // Pre-calculate all weekly mileages and long runs (working backwards)
...
function calculate5kLongRun(weeklyMileage, weeksUntilRace) {
...
function calculateTargetPeak5k(startingMileage, trainingWeeks, level) {
    let basePeak;
