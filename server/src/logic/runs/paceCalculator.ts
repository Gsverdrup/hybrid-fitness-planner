...
export function applyPacesToPlan(racePlan: WeeklyPlan[], profile: FitnessProfile): void {
    ...
    const buildWeeks: number = racePlan.filter(week => week.weekType === "B").length;
    ...
    for (let weekIdx = 0; weekIdx < racePlan.length; weekIdx++) {
        ...
        let runDays: DailyPlan[] = weekPlan.days.filter(day => day.workouts.some(w => w.type === "run"));
...
export function populatePaceChart(profile: FitnessProfile): void {
...
function calculatePaceImprovementDelta(profile: FitnessProfile): number {
